...
export default class DFS extends SearchTemplate {
    ...
    for (let i = 0; i < this.c.length; i++) {
      for (let j = 0; j < this.c[0].length; j++) {
        if (this.c[i][j]?.toString() === this.p5.color(0, 255, 0).toString()) {
          this.start = { row: i, col: j, parent: null }; // Set parent as null for the start cell
        }
        if (this.c[i][j]?.toString() === this.p5.color(255, 0, 0).toString()) {
          this.end = { row: i, col: j, parent: null }; // No need for parent for end cell
        }
      }
    }
  ...
  getNeighbors(cell: Cell): Cell[] {
    ...
    const addNeighbor = (newRow: number, newCol: number) => {
      if (
        ...
        const neighbor = { row: newRow, col: newCol, parent: cell }; // Track parent for backtracking
  ...
  check(): void {
    if (this.stack.length > 0 && !this.targetReached) {
  ...
  draw(): void {
    for (const { row, col } of this.currentPath) {
      if (!this.targetReached) {
        this.c[row][col] = this.p5.color(128, 128, 128); // Gray for active exploration path
      }
    }
