...
interface Cell {
  row: number;
  col: number;
  h: number; // Heuristic cost to the target
  parent: Cell | null; // Reference to the parent cell for backtracking
}


export default class GreedyBestFirst extends SearchTemplate {
    ...
    for (let i = 0; i < this.c.length; i++) {
      for (let j = 0; j < this.c[0].length; j++) {
        if (this.c[i][j]?.toString() === this.p5.color(0, 255, 0).toString()) {
          this.start = { row: i, col: j, h: 0, parent: null }; // Set parent as null for start cell
        }
        if (this.c[i][j]?.toString() === this.p5.color(255, 0, 0).toString()) {
          this.end = { row: i, col: j, h: 0, parent: null };
        }
      }
    }
  
    if (this.start) {
      this.start.h = this.heuristic(this.start); // Calculate heuristic for the start
  ...
  getNeighbors(cell: Cell): Cell[] {
    ...
    const addNeighbor = (newRow: number, newCol: number) => {
      if (
        ...
        const neighbor = { row: newRow, col: newCol, h: this.heuristic({ row: newRow, col: newCol, h: 0, parent: null }), parent: cell }; // Set parent for backtracking
  ...
  check(): void {
    if (this.openList.length > 0 && !this.targetReached) {
      // Sort open list by lowest heuristic value (h)
      this.openList.sort((a, b) => a.h - b.h);
